...
alert(elem.style.position.indexOf('sticky'))
if (elem.style.position.indexOf('sticky') < 0) {
  // 当前浏览器不支持粘性定位，需要 fallback 实现
  Array.prototype.forEach.call(document.querySelectorAll('.fixed'), function (elem) {
    elem.style.height = elem.clientHeight + 'px';
  });
  addEventListener('scroll', function() {
    var stickyElements = document.querySelectorAll('.fixed');
